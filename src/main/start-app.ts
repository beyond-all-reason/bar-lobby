...
export interface StartAppContext {
    setSecondInstanceHandler(handler: (argv: string[], workingDirectory: string) => void): void;
}
...
export async function startApp(context: StartAppContext) {
    ...
    navigationService.registerIpcHandlers(mainWindow, webContents);
    pathsService.registerIpcHandlers(webContents);

    navigationService.enqueueReplayRequest(process.argv, process.cwd());
    context.setSecondInstanceHandler((argv, workingDirectory) => {
        navigationService.handleSecondInstance(argv, workingDirectory);
    });
}
