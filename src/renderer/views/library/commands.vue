  ...
  switch (sortMethod) {
    ...
    case "Description":
      return commands.sort((a, b) =>
        a.cmdDescription.localeCompare(b.cmdDescription)
      );
    default:
      return commands;
  }
...
const filteredCommands = computed(() => {
  ...
  return sortCommands(
    commands.filter(
      (command) =>
        (command.cmd + command.cmdDescription).includes(searchVal.value)
    ),
    sortMethod.value
  );
});
...
onMounted(() => {
  ...
  api.comms
    .onResponse("s.communication.received_direct_message")
    .add(async (data) => {
      const { message } = data;
