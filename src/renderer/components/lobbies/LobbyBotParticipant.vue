...
import type { LobbyBot } from "@renderer/components/lobbies/lobby.types";
...
const hostName = computedAsync(async () => {
    ...
    if (props.hostId) {
        const cached: User = (await db.users.get(props.hostId)) as User;
        if (cached != undefined) {
...
function submitBotOptions() {
    ...
    if (Object.keys(optionsToSubmit).length != 0) {
        for (const optionKey in optionsToSubmit) {
            ...
            if (props.bot.options && props.bot.options[optionKey] != optionsToSubmit[optionKey]) {
