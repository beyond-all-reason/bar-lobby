...
<template>
    <div key="spectators" class="group" data-type="group" @dragenter.prevent="onDragEnter($event)" @dragover.prevent @drop="onDrop($event)">
        <div class="flex-row flex-center-items gap-md">
            ...
            <div v-if="memberCount > 0" class="member-count">({{ memberCount }} {{t('lobby.components.battle.spectatorsComponent.member')}}{{ memberCount > 1 ? "s" : "" }})</div>
            <Button v-if="showJoin" class="slim black" @click="onJoinClicked()"> {{ t('lobby.components.battle.spectatorsComponent.join') }} </Button>
        </div>
